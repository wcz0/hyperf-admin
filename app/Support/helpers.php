...
if (!function_exists('admin_url')) {
    ...
    {
        $prefix = $needPrefix ? '/' . \App\Admin::config('admin.route.prefix') : '';
...
if (!function_exists('admin_resource_full_path')) {
    function admin_resource_full_path($path, $server = null)
    {
        ...
        if (url()->isValidUrl($path) || mb_strpos($path, 'data:image') === 0) {
            ...
            $disk = \App\Admin::config('admin.upload.disk');
        ...
        $scheme = 'http:';
        if (\App\Admin::config('admin.https', false)) {
            $scheme = 'https:';
        }
...
if (!function_exists('settings')) {
    function settings()
    {
        return \App\Services\AdminSettingService::make();
    }
}
...
if (!function_exists('admin_pages')) {
    ...
    {
        return \App\Services\AdminPageService::make()->get($sign);
    }
}
